...
SERVICE_PACKAGE="flowstate_ensenso"
...
if [[ -n "$CI" || "$PRESUBMIT" == "true" ]]; then
  ...
  CONTEXT_ARGS+=(
    ...
    --build-context "ensenso_underlay=docker-image://ghcr.io/${GITHUB_OWNER}/${CAMERA_TYPE}-underlay:latest"
  )
else
