      ...
      capture_params_(ZividCaptureParameters::boot_defaults()) 
{
  ...
  this->declare_parameter<std::string>("settings_yaml", ""); // For zivid_camera node
  ...
  zivid_camera_param_client_ = std::make_shared<rclcpp::AsyncParametersClient>(this, zivid_ns + "/" + zivid_node_name);
  RCLCPP_INFO(get_logger(), "Waiting for zivid_camera parameter service to appear...");
  if (!zivid_camera_param_client_->wait_for_service(std::chrono::seconds(10))) {
      RCLCPP_ERROR(get_logger(), "Timeout waiting for zivid_camera parameter service. This may lead to parameter setting failures.");
      throw std::runtime_error("zivid_camera parameter service not available.");
  }

  RCLCPP_INFO(get_logger(), "Applying initial default settings to zivid_camera node.");
  const auto initial_settings_yaml = generateZividSettings();
  zivid_camera_param_client_->set_parameters({rclcpp::Parameter("settings_yaml", initial_settings_yaml)});

  set_parameters_callback_handle_ = this->add_on_set_parameters_callback(
    std::bind(&AdapterNode::setParametersCallback, this, std::placeholders::_1));
  ...
  color_image_sub_ = create_subscription<sensor_msgs::msg::Image>(
      ColorImageTopic(), 2, [this](sensor_msgs::msg::Image::UniquePtr msg) {
        ...
        }
        {
          ...
          std::lock_guard<std::mutex> snapshot_lock(this->snapshot_mutex_, std::adopt_lock);
          std::lock_guard<std::mutex> data_lock(this->data_mutex_, std::adopt_lock);
  ...
  snapshot_service_ = create_service<Snapshot>(
      ...
      rmw_qos_profile_services_default, callback_group_);
  RCLCPP_INFO(this->get_logger(), "zivid_node_ = %s", zivid_node_->get_fully_qualified_name());
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  for (const auto & param : parameters) {
    RCLCPP_INFO_STREAM(
      get_logger(), "AdapterNode: Setting parameter '" << param.get_name() << "' (" << param.get_type_name()
    ...
    if (is_individual_param) {
      ...
      RCLCPP_INFO_STREAM(get_logger(), "Passing through parameter '" << param.get_name() << "' to zivid_camera node: " << zivid_node_->get_fully_qualified_name());
      zivid_camera_param_client_->set_parameters({param});
    }
  }

  if (individual_param_changed && !full_yaml_provided) {
    RCLCPP_INFO(get_logger(), "Individual parameter changed. Generating and applying new settings_yaml.");
    const auto settings_yaml = generateZividSettings();
    zivid_camera_param_client_->set_parameters({rclcpp::Parameter("settings_yaml", settings_yaml)});
  }
...
{
  RCLCPP_INFO_STREAM(get_logger(), "Generating Zivid settings from current capture parameters.");
  ...
  auto add_acquisition_to_yaml = [&](double exposure_us) {
    yaml << "    - Acquisition:\n"
         << "        Aperture: " << capture_params_.aperture << "\n"
         << "        Brightness: " << capture_params_.projector_brightness << "\n"
         << "        ExposureTime: " << static_cast<long long>(exposure_us) << "\n"
...
void AdapterNode::DescribeCallback(
  ...
  
  {
    ...
    if (!camera_info_) {
      RCLCPP_INFO(get_logger(), "CameraInfo not available, triggering capture.");
      auto request = std::make_shared<std_srvs::srv::Trigger::Request>();
      auto future = capture_client_->async_send_request(request);
      if (future.wait_for(std::chrono::seconds(10)) != std::future_status::ready)
      {
          response->error_message = "Failed to trigger capture on zivid_camera node (timeout)";
      ...
      if (!result->success)
      {
          response->error_message = "Capture on zivid_camera node failed: " + result->message;
          response->success = false;
          RCLCPP_ERROR(get_logger(), response->error_message.c_str());
          return;
      }
  ...
  std::lock_guard<std::mutex> lock(camera_info_mutex_);
  if (!camera_info_ ) {
    response->error_message = "CameraInfo not yet received from Zivid camera after capture";
    response->success = false;
    RCLCPP_ERROR(get_logger(), response->error_message.c_str());
    return;
  }
...
void AdapterNode::SnapshotCallback(
  ...
  RCLCPP_INFO(get_logger(), "Capture trigger successful, waiting for images...");

  {
    ...
    if (!snapshot_cv_.wait_for(lock, std::chrono::seconds(5), [this] {
          ...
          return new_color_image_received_ && new_depth_image_received_ && new_normal_pc_received_;
        })) {
      response->error_message = "Did not receive images from zivid_camera node within timeout";
      response->success = false;
      RCLCPP_ERROR(get_logger(), response->error_message.c_str());
      return;
    }
  ...
  if (!color_image_ || !depth_image_) {
    ...
    response->error_message = "Color or depth images not yet received from camera";
    response->success = false;
    RCLCPP_ERROR(get_logger(), response->error_message.c_str());
    return;
  }
  if (!normal_pc_) {
    response->error_message = "Normals point cloud not yet received from camera";
    response->success = false;
    RCLCPP_ERROR(get_logger(), response->error_message.c_str());
    return;
  }
  ...
  if (depth_snapshot.image.encoding != sensor_msgs::image_encodings::TYPE_32FC1) {
    RCLCPP_WARN(get_logger(), "Zivid depth image encoding is not TYPE_32FC1C1 as expected.");
  }
